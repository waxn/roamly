...
**Mobile tracking:** The Android app (`mobile/`, Kotlin + Jetpack Compose) uses a claymorphism design system (`ui/theme/Theme.kt` + `ui/theme/Clay.kt`: soft dual-shadow surfaces, gradient accents, puffy press-animated buttons). On Android 12+ the theme applies Material You **dynamic color** (`dynamicDarkColorScheme` / `dynamicLightColorScheme`) for surface/background tones while keeping the Roamly coral/teal/violet brand accents. `ClayCard`/`ClayButton` borders are 0.5dp (was 1dp) with reduced alpha for a less shiny look. Login is **session-based** — logging in stores the session cookie and does **not** create or require an API key. A device id is auto-assigned at login. The API key is needed **only for tracking** (it authenticates location uploads) and is set up lazily: when the user starts tracking without one, Settings prompts them to create it in-app (idempotent `POST /api/keys/app/`, which returns the account's single existing active key or mints one named "Roamly Android") or to paste an existing key. There is **just one** key and it works forever (keys don't expire), so re-logging-in never spawns duplicates. `ApiKeyAuthMiddleware` authenticates the Bearer key on every endpoint, so once a key exists the app also stays usable via the key even if the session cookie lapses. `isLoggedIn` (in `AuthViewModel`) keys off server URL + session id.

The location tracker (`tracking/LocationTrackingService.kt`) is a foreground service that survives reboots and OS kills via a durable `trackingEnabled` intent flag (distinct from the runtime `trackingActive` flag), START_STICKY, and the boot receiver. **Capture uses two layers that cover each other** (`applyCapture`), because no single mechanism is reliable across all device states:

- **Primary: the continuous `requestLocationUpdates` stream + a pinned `PARTIAL_WAKE_LOCK`**, kept up whenever tracking is on and not paused (screen on *or* off). It delivers a smooth per-interval cadence whenever the device is **not in deep Doze** — screen-on, **charging**, or **moving** (motion keeps many devices out of Doze). This is what actually achieves a tight 15s cadence: on some devices (e.g. GrapheneOS/microG) the OS throttles even exact `setExactAndAllowWhileIdle` alarms to **~once per minute** screen-off *regardless of the battery-optimization whitelist*, so an alarm-per-fix model alone can't hold a short interval there — the stream can. Keeping the stream up screen-off is self-limiting on battery: deep Doze suspends the stream and ignores the wake lock, so the heavy path only runs when the device is already awake.
- **Backup: the exact-alarm cadence** (`runFixCycle` → `scheduleNextFix`), armed at every interval, that carries through **deep Doze** where the stream is suspended. Each cycle re-enters the service via an exact `setExactAndAllowWhileIdle(ELAPSED_REALTIME_WAKEUP)` alarm + `ACTION_TAKE_FIX` (a `getForegroundService` PendingIntent — an FGS-start exemption that also revives a killed service). `runFixCycle` **skips cheaply while the stream is keeping the interval fresh** (`streaming && a point landed within the interval`) and **takes over the moment the stream goes quiet** (Doze). The alarm cadence is clamped to `MIN_ALARM_FLOOR_MS` (15s) since GPS cold-starts after Doze can take 15–30s; next-fix delay is measured from cycle *start* (`floor − elapsed`) so the seconds spent acquiring count toward the interval and points land every ~interval, not interval+budget.
- **Best-fix acquisition, not first-fix-or-discard (`acquireBestFix`).** Accuracy is a **target, not a hard reject**. Each backup cycle streams fixes for a bounded `budget` (`min(interval, ACQUIRE_BUDGET_MAX_MS=10s)`) and keeps the *most accurate* one, stopping early when a fix meets the user's `maxAccuracyM` target, or when accuracy **plateaus** (`ACQUIRE_STALL_MS` 3.5s with no improvement — the hardware can't do better here, so stop burning GPS), or at budget end. It then logs the best fix it got rather than gapping. (The old hard accuracy filter silently dropped *every* indoor/stationary fix when the user set a tight cutoff — e.g. 17m fixes vs a 10m limit — which was a real cause of "captured-point" gaps.)
- **Dwell points (`dwellPointFrom`).** When a cycle gets **no fresh fix** — the fused provider stationary-throttles screen-off and keeps handing back the *same cached location*, which the dedup rejects — it re-logs the last known position stamped with the current time (zero speed), so a stationary device still records one point per interval (the dwell feature) instead of gapping. Only while the last real fix is recent (`DWELL_MAX_AGE_MS` 10 min).
- **Interval-scaled catch-up backstop.** Every `scheduleNextFix` arms a *second*, independent Doze-piercing exact alarm (`FIX_CATCHUP_REQUEST_CODE`) at `CATCHUP_MULTIPLIER` (3) × the interval, re-armed on every fix, so a dropped primary alarm / wedged cycle / hard kill recovers within **3×** instead of waiting for the coarse 15-min heartbeat. `runFixCycle` also force-resets a `fixInProgress` flag stuck longer than the acquisition budget, so a hung provider callback can't silently stall the loop.

Tracking knobs (interval, GPS priority auto/high/balanced/low, max accuracy) are observed as a flow and applied live via `applyCapture`. Acquisition callbacks run on a dedicated `HandlerThread` ("RoamlyLocCb") — not the main looper — so they arrive promptly even when OEM firmware throttles the service's main thread. When the backup cycle gets no fix at all, it retries through **three tiers**: quick (`FIX_RETRY_DELAY_MS` 4 s × `MAX_FAST_RETRIES` 3), medium (`MEDIUM_RETRY_DELAY_MS` 10 s × `MAX_MEDIUM_RETRIES` 3), then the `MIN_ALARM_FLOOR_MS` floor. When priority is "auto" the fix **degrades from `HIGH_ACCURACY` to `BALANCED` after just 2 misses** so network/WiFi supplies a coarse fix; explicit `high`/`balanced`/`low` are honoured exactly, and **HIGH/satellite stays the first attempt so airplane mode (GPS-only, no network) still gets precise fixes**. `acquireBestFix` falls back to `fusedClient.lastLocation` when nothing streams in the budget. A coarse 15-min `setExactAndAllowWhileIdle` heartbeat (`TrackingAlarmReceiver`) remains as the ultimate deep-Doze "is the service even alive" liveness check, beneath the catch-up alarm.

**Battery exemption is gated, not optional.** Without the Doze battery-optimization exemption Android throttles even `setExactAndAllowWhileIdle` to ~once per 9 min in Doze, so the backup cadence collapses. Starting tracking is therefore **blocked behind a required exemption gate** in `SettingsScreen` (the system grant dialog auto-continues into tracking; an explicit **"Track anyway (gaps)"** escape hatch remains). While not exempt, a persistent coral warning banner shows under the tracking hero and the foreground-service notification (`BigTextStyle`) appends a "⚠ Battery optimization on — tracking may have gaps" line. The exemption state is re-checked on every Settings resume (`refreshBatteryOptimizationState`). **`LocationFilter` is the single accept() authority and is never movement-gated *or* accuracy-gated** (a stationary user still logs one point per interval — those stacked dots are the dwell feature; accuracy is handled as a target in `acquireBestFix`, not a reject); it rejects fixes that are stale (older than `2× interval`), a duplicate/older timestamp than the last accepted fix, or a physically impossible **teleport** (implied speed > 357 m/s ≈ 1285 km/h — a GPS glitch), plus a purely time-based de-dup at the interval. Points are cached in Room and uploaded by `UploadWorker` (offline-first; `scheduleNow(replace=true)` for user-initiated syncs so a job stuck in retry-backoff doesn't swallow the tap); a local CSV mirror is also written. **Batch upload:** `UploadWorker` tries `POST /api/push/batch/` first (up to 100 points in one HTTP call, server uses `bulk_create(ignore_conflicts=True)`); falls back to single-point `POST /api/push/` on 404/405 so it works against older server instances. A 2-hour offline gap at the 30 s default (≈ 240 points) uploads in 3 batch calls instead of 240 single calls. **Wedged-backoff recovery:** automatic point-driven syncs enqueue with KEEP, so a failed `UploadWorker` run that lands in WorkManager's exponential backoff would otherwise no-op every later schedule and starve uploads for up to an hour (a cause of server-side delivery gaps even screen-on — the classic "nothing uploaded for ages while driving" hole, since WorkManager's backoff is *time-based* and keeps the job asleep for minutes even after signal returns). `savePoint` watches the unsynced backlog and escalates that cycle's `scheduleNow` to `replace=true` to cancel the stuck job and run fresh. The **primary trigger is time-based and interval-independent**: a non-empty backlog plus no *successful* delivery for `UPLOAD_STUCK_AGE_MS` (120s; read from `prefs.lastSyncTime`/`lastSyncSuccess`, which `UploadWorker` writes after every run). The old count-only rule (`UPLOAD_STUCK_THRESHOLD` ~40 points) is kept only as a backstop ceiling, because a raw count scales badly with the interval — 40 points is ~6 min at a 10s track but ~20 min at the 30s default, far too slow. Both are gated on the 60s time threshold so REPLACE recurs at most once per cycle and never interrupts a healthy in-flight flush. As a second safety net, `MainActivity.onResume` force-pushes (`replace=true`) any unsynced backlog when the app is foregrounded — previously `onResume` only *pulled* the server delta into the local store, so reopening the app after a drive showed fresh local points that hadn't reached the server.
...
4. *Doze-piercing alarms* — two distinct exact `setExactAndAllowWhileIdle` + `ELAPSED_REALTIME_WAKEUP` alarms (both fall back to inexact `setAndAllowWhileIdle` when exact-alarm permission is denied): (a) the **per-point fix alarm** scheduled inside the service (`scheduleNextFix`, `ACTION_TAKE_FIX`) that carries the capture cadence through Doze in alarm mode, and (b) the coarse **15-min liveness heartbeat** (`receiver/TrackingAlarmReceiver`) on a self-rescheduling loop that re-checks the service is alive even in deep Doze. Exact-alarm-triggered FGS starts are an FGS-start exemption, so both also serve as reliable restart paths.
...
8. *Multi-provider re-arm* — FusedLocation already fuses GPS + network + passive; a dynamically-registered `PROVIDERS_CHANGED` receiver in the service re-arms capture immediately when providers toggle (a fresh fix cycle in alarm mode, or restarting the stream in continuous mode). `LocationTrackingService.start()/stop()` are wrapped in `runCatching` so a background FGS-start blocked on Android 12+ degrades to a heartbeat retry instead of crashing.
...
**Mobile local-first location store:** The map and "have I been here?" no longer hit the network per screen — the full location history is mirrored on-device and queried locally. `data/local/LocationStore.kt` (singleton) backs a Room table (`synced_locations`, keyed by the server's location id so re-syncing is a dedup upsert) in `TrackingDatabase` (v2; a real `MIGRATION_1_2` adds the table without wiping the `cached_points` upload queue). Sync is **incremental**: it pulls only points newer than the newest local one, paging ascending via the locations API's existing cursor (`sort_dir=asc` + `before_value`/`before_id`, plus the `has_more`/`next_before_value`/`next_before_id` response fields). `sync()` is throttled and runs on app foreground (`MainActivity.onResume`) and on `MapViewModel` init. `MapViewModel` also observes `store.state` (a `StateFlow<SyncState>`) and reloads points whenever a new sync completes — so returning to the app after traveling automatically repaints the map with fresh data. The map paints instantly from the store: a decimated period overview (`id % stride` sampling keeps the heatmap fast) plus full-resolution viewport dots pulled from the local DB on pan/zoom (no per-move request). "Have I been here?" scans the **entire** local history around the user — instant and complete, fixing the old under-count that came from a server result-set limit. `SyncedLocationDao` also has `inTimeRange(startMs, endMs)` to support the custom date range view. The store is wiped on sign-out alongside the disk cache.

**Mobile feature parity:** The app mirrors the web's core consumer features — Map (time periods + **custom date range picker**, heatmap, "have I been here?"), Adventures, Pals, Stats (with yearly overview + top places), Journals, Settings (including a **Diagnostics** page), and history Search (`ui/search/`, hits `/api/search/` — text queries return named POI matches that focus the map plus the days you visited a matching city/state/place; **tapping a day row filters the map to that date**). The map's `TimePeriod.CUSTOM` option opens a Material3 `DateRangePicker` dialog and stores a `DateRange(start, end)` in `MapUiState`; the ViewModel calls `LocationStore.timeRange()` for those. `MapViewModel.navigateToDate(dateStr)` sets a custom single-day range and is called from search day clicks and journal track-card taps. The `MapViewModel` is scoped to the Activity (created outside any NavBackStackEntry in `RoamlyNavHost`) so Journal can trigger date navigation on it. Journals (`ui/journals/`) are a bottom-nav destination: a Monday-first month calendar (entry days show the mood emoji / favorite star / photo dot), a recent-entries list, streak/lifetime stats, and a full-screen editor (title, mood/weather emoji pickers, favorite toggle, debounced-autosave body, Coil-loaded photo grid with picker upload, and that day's GPS track drawn on a Compose Canvas — the **Canvas math was fixed** (correct aspect-ratio-preserving normalization + gap-breaking for multi-segment days) and tapping the track card navigates to the map filtered to that date). "Have I been here?" shows **time spent** at each past visit (computed from consecutive point timestamps, capped 1 h per gap like the server), and tapping a past day filters the map to that date. Power-user/admin features (data table, import/export, backups, geocoding/POI jobs, API-key CRUD, profile pictures) remain web-only.
