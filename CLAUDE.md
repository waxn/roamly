...
**Geocoding is never inline on the push path.** `push_location` (`/api/push/`) creates points with `city=''` and returns immediately; labelling happens in the background. A blocking Nominatim reverse-geocode per point (up to 10s, rate-limited above ~1 req/sec) used to run inside the request — with the mobile uploader pushing one point per request, that stalled uploads past the client timeout and wedged WorkManager backoff, producing multi-minute-to-hour delivery gaps even screen-on. New points are now labelled by the existing rate-limited cluster worker, kicked fire-and-forget via `geocoding_tasks.ensure_auto_geocode(user_id)` (debounced per user, no-op while a geocode thread — manual or auto — is already running). The same worker also still runs on demand from Settings.
