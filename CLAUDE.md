...
The location tracker (`tracking/LocationTrackingService.kt`) is a foreground service that survives reboots and OS kills via a durable `trackingEnabled` intent flag (distinct from the runtime `trackingActive` flag), START_STICKY, and the boot receiver. **Capture runs in one of two modes, chosen by `applyMode` from the interval *and* whether the app is battery-optimization-exempt** (continuous ceiling = `WARM_CONTINUOUS_MAX_MS` 20s if exempt, else `CONTINUOUS_MAX_INTERVAL_MS` 5s):

- **Alarm mode (interval above the ceiling — e.g. the 30s default, or anything >5s when not exempt).** This is the **GPSLogger model** and the fix for the old "works for a few minutes then stalls for minutes, screen on or off" bug. Each cycle (`runFixCycle`) acquires a short **time-boxed** `PARTIAL_WAKE_LOCK`, requests **one** fresh fix via `fusedClient.getCurrentLocation` (high accuracy for `auto`; honours explicit `high`/`balanced`/`low`) bounded by an acquisition timeout, saves it, releases the lock, then schedules the **next** fix with an exact `setExactAndAllowWhileIdle(ELAPSED_REALTIME_WAKEUP)` alarm that re-enters the service via `ACTION_TAKE_FIX` (a `getForegroundService` PendingIntent straight to the service — no extra component). **The exact alarm is the per-point heartbeat**, so the CPU can sleep between fixes and still wake on time (and `getForegroundService` from an exact alarm is an FGS-start exemption, so it also revives the service if the OS killed it). This replaced a **continuous `FusedLocationProviderClient` stream**, which the OS throttles in the background: it delivered a burst of points right after start then went silent, because continuous updates are suspended while the CPU sleeps **and** the `delay()`-based watchdog meant to re-arm them is itself frozen during Doze.
- **Continuous mode (interval ≤ the ceiling).** A live `requestLocationUpdates` stream + an **always-pinned** `PARTIAL_WAKE_LOCK` + the `delay()` watchdog re-arm. It keeps the GPS **warm** (no cold-start per fix → tighter, smoother cadence) but only stays alive screen-off while the CPU is held awake — which is why it's gated to short intervals, and extended to ≤20s only for exempt apps (which can hold the CPU through Doze). For a 10s track on an exempt phone this is the smooth path; alarm mode's cold-started fixes are choppier there. The `LocationRequest` sets `minUpdateDistanceMeters(0)` / no batching / no wait-for-accurate so a fix arrives every interval regardless of movement.

Tracking knobs (interval, GPS priority auto/high/balanced/low, max accuracy) are observed as a flow and applied live; an interval change crossing the continuous ceiling switches modes (`applyMode`). Alarm mode uses `requestLocationUpdates(maxUpdates=1)` on a dedicated `HandlerThread` ("RoamlyLocCb") — not `getCurrentLocation()` or the main looper — so callbacks arrive promptly even when OEM firmware throttles the service's main thread. Misses progress through **three retry tiers**: quick (`FIX_RETRY_DELAY_MS` 4 s × `MAX_FAST_RETRIES` 3), medium (`MEDIUM_RETRY_DELAY_MS` 10 s × `MAX_MEDIUM_RETRIES` 3), then the normal interval — the medium tier covers GPS warm-up time after Doze (up to 30 s TTFF). When priority is "auto" and misses exceed `MAX_FAST_RETRIES`, the fix request degrades from `HIGH_ACCURACY` to `BALANCED` so network/WiFi location fills indoor gaps. Both continuous mode and the watchdog re-arm also use the same HandlerThread looper. A coarse 15-min `setExactAndAllowWhileIdle` heartbeat (`TrackingAlarmReceiver`) remains as the deep-Doze "is the service even alive" backstop, separate from the per-point fix alarm. **`LocationFilter` is the single accept() authority and is never movement-gated** (a stationary user still logs one point per interval — those stacked dots are the dwell feature); it rejects fixes that are stale (older than `2× interval`), too inaccurate (> `maxAccuracyM`), a duplicate/older timestamp than the last accepted fix, or a physically impossible **teleport** (implied speed > 357 m/s ≈ 1285 km/h — a GPS glitch), plus a purely time-based de-dup at the interval. Points are cached in Room and uploaded by `UploadWorker` (offline-first; `scheduleNow(replace=true)` for user-initiated syncs so a job stuck in retry-backoff doesn't swallow the tap); a local CSV mirror is also written. **Batch upload:** `UploadWorker` tries `POST /api/push/batch/` first (up to 100 points in one HTTP call, server uses `bulk_create(ignore_conflicts=True)`); falls back to single-point `POST /api/push/` on 404/405 so it works against older server instances. A 2-hour offline gap at the 30 s default (≈ 240 points) uploads in 3 batch calls instead of 240 single calls. **Wedged-backoff recovery:** automatic point-driven syncs enqueue with KEEP, so a failed `UploadWorker` run that lands in WorkManager's exponential backoff would otherwise no-op every later schedule and starve uploads for up to an hour (a cause of server-side delivery gaps even screen-on — the classic "nothing uploaded for ages while driving" hole, since WorkManager's backoff is *time-based* and keeps the job asleep for minutes even after signal returns). `savePoint` watches the unsynced backlog and escalates that cycle's `scheduleNow` to `replace=true` to cancel the stuck job and run fresh. The **primary trigger is time-based and interval-independent**: a non-empty backlog plus no *successful* delivery for `UPLOAD_STUCK_AGE_MS` (120s; read from `prefs.lastSyncTime`/`lastSyncSuccess`, which `UploadWorker` writes after every run). The old count-only rule (`UPLOAD_STUCK_THRESHOLD` ~40 points) is kept only as a backstop ceiling, because a raw count scales badly with the interval — 40 points is ~6 min at a 10s track but ~20 min at the 30s default, far too slow. Both are gated on the 60s time threshold so REPLACE recurs at most once per cycle and never interrupts a healthy in-flight flush. As a second safety net, `MainActivity.onResume` force-pushes (`replace=true`) any unsynced backlog when the app is foregrounded — previously `onResume` only *pulled* the server delta into the local store, so reopening the app after a drive showed fresh local points that hadn't reached the server.
