...
**AI day summaries** (`AISummary`, `tracker/ai_tasks.py`) ask the configured OpenAI-compatible LLM for a JSON object — a concise 1–4 sentence first-person `summary` plus 3–6 descriptive `tags` (place types / activities / mood) — built from the day's cities, dwelled-at POIs (from `Visit`) and the journal entry. `_parse_summary_response` extracts the JSON robustly (code-fence/brace stripping, prose fallback). Tags are stored in `AISummary.tags_json` (migration `0024`) and rendered as chips on the Summaries page. The journal day track (`_journal_day_track`) accepts a `tz_offset` (browser `getTimezoneOffset()` minutes) so the journal map and AI summary bound to the user's **local** calendar midnight, not the server's UTC day.
