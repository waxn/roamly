...
The location tracker (`tracking/LocationTrackingService.kt`) is a foreground service that survives reboots and OS kills via a durable `trackingEnabled` intent flag (distinct from the runtime `trackingActive` flag), START_STICKY, and the boot receiver. **Capture mode is chosen by `applyCapture` from *screen state*, not the interval** — because the thing that actually breaks screen-off tracking is **deep Doze suspending the continuous location stream itself** (the OS batches/suspends streamed updates in Doze no matter how many wake locks are held — that suspension is what caused the historical multi-minute screen-off gaps, even on a battery-optimization-exempt phone). So:

- **The exact-alarm cadence is the ALWAYS-ON, Doze-proof floor**, armed at *every* interval whenever tracking is on and not paused. Each cycle (`runFixCycle`) acquires a short **time-boxed** `PARTIAL_WAKE_LOCK`, requests **one** fresh fix via `requestLocationUpdates(maxUpdates=1)` on a dedicated `HandlerThread` ("RoamlyLocCb"), saves it, then schedules the **next** fix with an exact `setExactAndAllowWhileIdle(ELAPSED_REALTIME_WAKEUP)` alarm that re-enters the service via `ACTION_TAKE_FIX` (a `getForegroundService` PendingIntent — an FGS-start exemption, so it also revives a killed service). The exact alarm **force-wakes the device out of Doze** to take each fix — the only mechanism that survives deep Doze screen-off. The alarm cadence is clamped to `MIN_ALARM_FLOOR_MS` (15s): GPS cold-starts after Doze can take 15–30s, so scheduling fixes faster just guarantees misses; the screen-on stream provides any finer rate. 15s keeps screen-off capture within **3× of even a 10s interval** (the hard gap ceiling).
- **The continuous warm stream is a *screen-on-only* add-on.** While the screen is on, Doze isn't engaged, so a live `requestLocationUpdates` stream + a **pinned** `PARTIAL_WAKE_LOCK` + the `delay()` watchdog give smooth high-rate capture; the always-on alarm idles (`runFixCycle` skips the cold GPS request when a stream point already landed within the interval, but **still reschedules**, so the cadence never dies). When the screen goes off (a dynamically-registered `ACTION_SCREEN_ON`/`ACTION_SCREEN_OFF` receiver drives `applyCapture`) the stream is torn down (Doze would suspend it anyway) and the alarm carries the cadence alone. The `LocationRequest` sets `minUpdateDistanceMeters(0)` / no batching / no wait-for-accurate so a fix arrives every interval regardless of movement.
- **Interval-scaled catch-up backstop.** Every `scheduleNextFix` arms a *second*, independent Doze-piercing exact alarm (`FIX_CATCHUP_REQUEST_CODE`) at `CATCHUP_MULTIPLIER` (3) × the interval, re-armed on every fix. If the primary fix alarm is ever dropped, the cycle wedges, or the service is hard-killed, this recovers the cadence within **3×** instead of waiting for the coarse 15-min heartbeat (the old single recovery path — the literal cause of the 5–20 min gaps). `runFixCycle` also force-resets a `fixInProgress` flag that's been stuck longer than the acquisition budget, so a hung provider callback can't silently stall the loop.

Tracking knobs (interval, GPS priority auto/high/balanced/low, max accuracy) are observed as a flow and applied live via `applyCapture` (which also re-runs on every screen on/off). The single-fix request uses `requestLocationUpdates(maxUpdates=1)` on the dedicated `HandlerThread` ("RoamlyLocCb") — not the main looper — so callbacks arrive promptly even when OEM firmware throttles the service's main thread. Misses progress through **three retry tiers**: quick (`FIX_RETRY_DELAY_MS` 4 s × `MAX_FAST_RETRIES` 3), medium (`MEDIUM_RETRY_DELAY_MS` 10 s × `MAX_MEDIUM_RETRIES` 3), then the `MIN_ALARM_FLOOR_MS` floor. When priority is "auto" the fix **degrades from `HIGH_ACCURACY` to `BALANCED` after just 2 misses** so network/WiFi location supplies a coarse fix and keeps the cadence rather than a slow GPS cold-start becoming a gap; explicit `high`/`balanced`/`low` are honoured exactly. As a further anti-gap measure, `requestSingleFix` falls back to `fusedClient.lastLocation` when the fresh request times out — a recent cached fix still anchors the cadence (the `LocationFilter` rejects it if actually stale/duplicate, so it can only help, never backdate). **HIGH/satellite stays the first attempt so airplane mode (GPS-only, no network) still gets precise fixes.** A coarse 15-min `setExactAndAllowWhileIdle` heartbeat (`TrackingAlarmReceiver`) remains as the ultimate deep-Doze "is the service even alive" liveness check, beneath the interval-scaled catch-up alarm.

**Battery exemption is gated, not optional.** Without the Doze battery-optimization exemption Android throttles even `setExactAndAllowWhileIdle` to ~once per 9 min in Doze, so the whole cadence collapses. Starting tracking is therefore **blocked behind a required exemption gate** in `SettingsScreen` (the system grant dialog auto-continues into tracking; an explicit **"Track anyway (gaps)"** escape hatch remains). While not exempt, a persistent coral warning banner shows under the tracking hero and the foreground-service notification (`BigTextStyle`) appends a "⚠ Battery optimization on — tracking may have gaps" line. The exemption state is re-checked on every Settings resume (`refreshBatteryOptimizationState`). **`LocationFilter` is the single accept() authority and is never movement-gated** (a stationary user still logs one point per interval — those stacked dots are the dwell feature); it rejects fixes that are stale (older than `2× interval`), too inaccurate (> `maxAccuracyM`), a duplicate/older timestamp than the last accepted fix, or a physically impossible **teleport** (implied speed > 357 m/s ≈ 1285 km/h — a GPS glitch), plus a purely time-based de-dup at the interval. Points are cached in Room and uploaded by `UploadWorker` (offline-first; `scheduleNow(replace=true)` for user-initiated syncs so a job stuck in retry-backoff doesn't swallow the tap); a local CSV mirror is also written. **Batch upload:** `UploadWorker` tries `POST /api/push/batch/` first (up to 100 points in one HTTP call, server uses `bulk_create(ignore_conflicts=True)`); falls back to single-point `POST /api/push/` on 404/405 so it works against older server instances. A 2-hour offline gap at the 30 s default (≈ 240 points) uploads in 3 batch calls instead of 240 single calls. **Wedged-backoff recovery:** automatic point-driven syncs enqueue with KEEP, so a failed `UploadWorker` run that lands in WorkManager's exponential backoff would otherwise no-op every later schedule and starve uploads for up to an hour (a cause of server-side delivery gaps even screen-on — the classic "nothing uploaded for ages while driving" hole, since WorkManager's backoff is *time-based* and keeps the job asleep for minutes even after signal returns). `savePoint` watches the unsynced backlog and escalates that cycle's `scheduleNow` to `replace=true` to cancel the stuck job and run fresh. The **primary trigger is time-based and interval-independent**: a non-empty backlog plus no *successful* delivery for `UPLOAD_STUCK_AGE_MS` (120s; read from `prefs.lastSyncTime`/`lastSyncSuccess`, which `UploadWorker` writes after every run). The old count-only rule (`UPLOAD_STUCK_THRESHOLD` ~40 points) is kept only as a backstop ceiling, because a raw count scales badly with the interval — 40 points is ~6 min at a 10s track but ~20 min at the 30s default, far too slow. Both are gated on the 60s time threshold so REPLACE recurs at most once per cycle and never interrupts a healthy in-flight flush. As a second safety net, `MainActivity.onResume` force-pushes (`replace=true`) any unsynced backlog when the app is foregrounded — previously `onResume` only *pulled* the server delta into the local store, so reopening the app after a drive showed fresh local points that hadn't reached the server.
