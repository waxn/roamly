...
def get_road_download_status():
    from .models import DownloadedRegion, RoadDownloadJob, RoadSegment

    total_ways = RoadSegment.objects.count()
    # How much of the history is already recorded as fetched. Reported so the UI
    # can tell "you have no location history" apart from "your history is fully
    # covered already" — both of which end a run with total=0 batches, and only
    # one of which means what the message used to say. See _visited_boxes.
    covered = DownloadedRegion.objects.filter(kind='road').count()
    try:
        job = RoadDownloadJob.objects.get(pk=1)
    except RoadDownloadJob.DoesNotExist:
